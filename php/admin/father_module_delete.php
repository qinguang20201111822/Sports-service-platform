...
$link=connect();
$query="select * from qg_son_module where father_module_id={$_GET['id']}";
$result=execute($link,$query);
if(mysqli_num_rows($result)){
    skip('father_module.php', 'error', '请先删除父板块内的所有子版块！');
}
//在 HTML 表单中使用 method="get" 时，所有的变量名和值都会显示在 URL 中
//在父页面进行删除时，要把删除的ID传到该界面，并进行删除语句
$query="delete from qg_father_module where id={$_GET['id']}";
execute($link,$query);
