...
if (isset($_POST['submit'])) {
    foreach ($_POST['sort'] as $key => $val) {
        ...
        $query[] = "update qg_son_module set sort={$val} where id={$key}";
    }
...
<div id="main" style="height:1000px;">
    ...
    <form method="post">
        ...
        <table class="list">
            ...
            <?php
            //利用$query把数据库给连接上
            $query = "select ssm.id,ssm.sort,ssm.module_name,ssm.member_id,sfm.module_name father_module_name from qg_son_module ssm,qg_father_module sfm where ssm.father_module_id=sfm.id order by sfm.id";
        ...
        */
            while ($data = mysqli_fetch_assoc($result)) {
                ...
                $message = "确定要删除子板块{$data['module_name']}吗？ ";

                $delete_url = "confirm.php?url={$url}&return.url={$return_url}&message={$message}";
                /*href 属性规定链接的目标地址：
